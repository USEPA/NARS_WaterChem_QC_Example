---
title: "National Lakes Assessment 2022 Water Chemistry and Chlorophyll a Validation"
subtitle: "Protocol and code for evaluating NLA 2022 lab water quality data for completeness, consistency, and quality"
date: "`r Sys.Date()`"
...
Appendix. This script is intended for use once all water chemistry
samples have been analyzed for the purposes of identifying potential
outliers or quality issues; however, this script may be used iteratively
to confirm observations are no longer flagged once data revisions are
implemented.
...
`r paste0("NARS_WaterChem_QA/reports/NLA_Water_Chem_QA_Report_", Sys.Date())`
called
...
are included for transparency in the quality assurance process.
...
# outDir <- paste0(here::here(), "/reports/NLA_Water_Chem_QA_Report_", Sys.Date())
...
# Create a date-stamped copy for use in QA process
...
msgSamp <- select(sampgrid, UID, SITE_ID, VISIT_NO, CHEM, CHLX) |>
  ...
  pivot_longer(cols = CHEM:CHLX, values_to = 'QA_MSG', names_to='SAMPLE_TYPE') |>
  filter(str_detect(QA_MSG, "[A-Z]"))
...
msgGrid <- anti_join(curSamp, sampgrid_long, by = c('UID', 'SITE_ID', 'VISIT_NO','SAMPLE_TYPE')) |>
  mutate(QA_MSG = "Individual analytes from this bulk sample appear in raw data, but are not included in sample grid")
...
msgData <- anti_join(sampgrid_long, curSamp, by = c('UID', 'SITE_ID', 'VISIT_NO','SAMPLE_TYPE')) |>
  select(-VALUE) |>
  mutate(QA_MSG = "Sample grid indicates no issues, but individual analytes from this bulk sample are not found in raw data")
...
# Consolidate chunk into one table for QA review
grid_check_fin <- msgComb |>
  dplyr::mutate(QA_TYPE = "Sample grid check", .after = SAMPLE_TYPE)
...
limit (RL). The checks completed in Table 4 include:
...
ck_na_res <- res_limits |>
  ...
  mutate(QA_MSG = "NA result without ND or NR flag")
...
comb_cks <- full_join(ck_mdl, ck_mdl.1) |>
  ...
  full_join(l_flag) |>
  unique() |>
  mutate(QA_MSG = ifelse(is.na(ND_RESULT), QA_MSG, ifelse(is.na(QA_MSG), ND_RESULT, paste0(ND_RESULT, "; ", QA_MSG)))) |>
  select(-ND_RESULT)

# Create final table for QA export
flag_checks_fin <- comb_cks |>
  select(UID, SITE_ID, VISIT_NO, ANALYTE, QA_MSG) |>
  dplyr::mutate(QA_TYPE = "Flag consistency check", .after = ANALYTE) |>
  dplyr::rename(SAMPLE_TYPE = ANALYTE)
...
qa_data_1 <- full_join(qa_res, qa_units, by = 'UID')

# We need to convert some analytes to ueq/L
qa_mod <- mutate(qa_data_1, 
                 SO4 = SULFATE*20.82,
                 NO3 = NITRATE_N*71.39,
                 NO2 = NITRITE_N*71.39,
...
qa_mod_2 <- group_by(qa_mod_1, UID) |> 
...
# Calculate balance as a percent of the sum of anion and cation charge. Calculating as a percent of the average of the two results in flagging many more observations than necessary for the QA process.
...
```{r ion-balance-test}
ionbal <- mutate(qa_mod_2,
                 OUTLIERION = 'NO',
                 TESTION = 'Non-outlier')

# Any observation with >20% balance should be flagged for QA review
...
iontest_fin <- iontest |>
  mutate(SAMPLE_TYPE = "IONS",
         QA_TYPE = "Ion balance",
         QA_MSG = paste0("DIRECTION = ", DIRECTION, " due to ", IMBAL_SRC, ". Check flags: ", ORGION_CHK, "; ", PH_CHK, "; ", ANC_CAMG_CHK, "; ", "NACL_CHK")) |>
  select(UID, SITE_ID, VISIT_NO, SAMPLE_TYPE, QA_TYPE, QA_MSG)
...
nutr_check_fin <- nutr_check |>
  mutate(SAMPLE_TYPE = "Nutrients",
         QA_TYPE = "Total & Dissolved Nutrients",
         QA_MSG = paste0(TN_NO3_NH4_FLAG, "; ", TN_TDN_FLAG, "; ", TP_TDP_FLAG)) |>
  select(UID, SITE_ID, VISIT_NO, SAMPLE_TYPE, QA_TYPE, QA_MSG)
...
## Export Data for QA Review

```{r output QA data}
# Combine all the QA checks
# Add columns to the data for QA reviewer to enter info
qa_flagged <- grid_check_fin |>
  bind_rows(missing_check_fin,
            flag_checks_fin,
            iontest_fin,
            cond_check_fin, 
            nutr_check_fin) |>
  mutate(QA_NOTES = " ", QA_ACTION = " ", QA_REVISIONS = " ")

file_name <- paste0(params$outDir, "/NLA22_WaterChem_ObservationsForReview_", Sys.Date(), ".csv")

# Write out the data
readr::write_csv(qa_flagged, file_name)

# Report the location of the exported data
loc <- sub(paste0(".*(", "/NARS_WaterChem_QA", ")"), "\\1", file_name)
```

A complete list of data flagged for QA review is exported to: `r loc`

Edit this tabular file to add QA notes, required actions, and necessary
...
analytes <- names(qa_data_1)[!(names(qa_data_1) %in% c('UID', 'VISIT_NO', 'SITE_ID', names(qa_data_1)[str_detect(names(qa_data_1), 'UNITS')]))] 

vis2 <- filter(qa_data_1, VISIT_NO=='2') |>
  select(-UID, -VISIT_NO) |>
  pivot_longer(cols = all_of(analytes), names_to='ANALYTE', 
               values_to='VISIT_2') |>
  select(SITE_ID, ANALYTE, VISIT_2)

vis1 <- filter(qa_data_1, VISIT_NO=='1') |>
  select(-UID, -VISIT_NO) |>
...
**quality assured (QAed)** (i.e., checked against other parameters,
field notes, repeat visits) to address values that are not feasible or
incorrect. In some cases, values that fail QA can be reanalyzed in the
lab or fixed by going back to field crews or other data; in other cases,
values that fail QA should be set to 'missing' or another value. The
