...
outDir <- paste0(here::here(), "/reports/Example_NLA_Water_Chem_QA_Report_", Sys.Date())
dir.create(path = outDir, showWarnings = FALSE, recursive = TRUE)

inDir_raw <- paste0(here::here(), "/data") 
inDir_data <- paste0(here::here(), "/data") 
fileout <- paste0("Example_NLA_Water_Chem_QA_Report_", Sys.Date(), ".html")

setwd("./code")
quarto_render(paste0(here::here(), "/code/NLA_QA_document.qmd"), 
              output_file = fileout, 
              output_format = 'html',
